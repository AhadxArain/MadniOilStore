...
const formatMessage = (text: string): string => {
  ...
  if (text.includes('Date:') && text.includes('Time:') && text.includes('Vehicle:')) {
    ...
    const lines = formatted.split(/(?:Date:|Time:|Vehicle:|Service:|Name:|Phone Number:|Email:)/);
    if (lines.length >= 7) {
...
Your appointment has been booked. You will also receive a confirmation email shortly.`;
  ...
  if (formatted.includes('hain:') && formatted.includes(' - ')) {
    ...
    if (parts.length >= 2) {
      ...
      const itemArray = listItems.split(' - ')
        .map(item => item.trim())
  ...
  if (formatted.includes(' hain: -') || formatted.includes('available') && formatted.includes(' - ')) {
    ...
    if (splitPoint !== -1) {
      const intro = formatted.substring(0, splitPoint + (formatted.charAt(splitPoint) === ':' ? 1 : 0)).trim();
      let listPart = formatted.substring(splitPoint + (formatted.charAt(splitPoint) === ':' ? 1 : 0)).trim();
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const isListItem = /^[A-Z][a-zA-Z0-9\s&%-]{1,30}$/.test(line) && 
                      !line.includes('?') && 
                      !line.includes('!') &&
    ...
    const suggestsList = prevLine.includes('hain:') || 
    ...
    if ((isListItem && (suggestsList || inPotentialList)) || currentListItems.length > 0) {
      if (isListItem) {
...
const ChatWidget = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent | null, initialMessage?: string) => {
    ...
    try {
      ...
      setMessages(prev => {
        ...
        const aiResponse = {
          ...
          text: formatMessage(responseText || "Got it! Please wait while I process your request."),
          sender: 'support',
          timestamp: new Date(),
        };
